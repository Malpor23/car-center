...
export class MecanicoComponent implements OnInit {
  ...
  getReload() {
    ...
    this.httpSv.getList(environment.apiURL + `mecanicos/paginate?pageNo=${pageNo}&pageZise=${pageZise}`).subscribe(
      resp => {
        this.listMecanicos = resp.content;
